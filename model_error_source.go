...
var AllowedErrorSourceEnumValues = []ErrorSource{
	"schematron",
	"pdfa",
	"pydantic",
	"xmp",
...
func (v *ErrorSource) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := ErrorSource(value)
	for _, existing := range AllowedErrorSourceEnumValues {
	...
	return fmt.Errorf("%+v is not a valid ErrorSource", value)
}

// NewErrorSourceFromValue returns a pointer to a valid ErrorSource
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewErrorSourceFromValue(v string) (*ErrorSource, error) {
	ev := ErrorSource(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for ErrorSource: valid values are %v", v, AllowedErrorSourceEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v ErrorSource) IsValid() bool {
	for _, existing := range AllowedErrorSourceEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to ErrorSource value
func (v ErrorSource) Ptr() *ErrorSource {
	return &v
}
