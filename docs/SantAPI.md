...
func main() {
	...
	resp, r, err := apiClient.SantAPI.HealthcheckHealthcheckGet(context.Background()).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `SantAPI.HealthcheckHealthcheckGet``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `HealthcheckHealthcheckGet`: interface{}
	fmt.Fprintf(os.Stdout, "Response from `SantAPI.HealthcheckHealthcheckGet`: %v\n", resp)
}
```
